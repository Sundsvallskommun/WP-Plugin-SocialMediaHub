 ...
 * @subpackage sk_social_media_hub/admin
 */
...
*/
  ...
  public function create_editor_page(){
    ...
    <h2 class="nav-tab-wrapper">  
      <a href="?page=sk-social-media-hub&amp;tab=start" class="nav-tab <?php echo $active_tab == 'start' ? 'nav-tab-active' : ''; ?>"><?php _e( 'Inställningar', 'sksmh' ) ?></a>  
      <a href="?page=sk-social-media-hub&amp;tab=display" class="nav-tab <?php echo $active_tab == 'display' ? 'nav-tab-active' : ''; ?>"><?php _e( 'Visning', 'sksmh' ) ?></a>  
      <?php if( current_user_can('manage_options' ) ) : ?>
        <a href="?page=sk-social-media-hub&amp;tab=advanced" class="nav-tab <?php echo $active_tab == 'advanced' ? 'nav-tab-active' : ''; ?>"><?php _e( 'Avancerat', 'sksmh' ) ?></a>
      <?php endif; ?>
    </h2>  
...
<?php if( $active_tab == 'start' ) : ?>
      ...
      <hr />
        ...
        <p class="submit"><input type="submit" name="update_sksmh" id="" class="button button-primary" value="<?php _e('Save Changes');?>"></p>
      </form>
      <?php endif; ?>

      
      <?php if( $active_tab == 'display' ) : ?>
        <div class="more"><?php _e('För att visa ett flöde använder du så kallade "shortcodes" där du har möjlighet att skicka med parametrar. En shortcode skrivs (eller kopieras in) direkt i den visuella editorn för en sida, inlägg eller puff.', 'sksmh'); ?></div>
      ...
      <?php if( $active_tab == 'advanced' ) : ?>
        <div class="more"><?php _e('För att erhålla nycklar och koder som nämns för tjänsterna nedan behöver du tillgång till respektive applikation för tjänsterna.', 'sksmh'); ?></div>
      <form method="post">
        <h2><?php _e('Instagram', 'sksmh' ); ?></h2>
        <table class="form-table">
          <tbody>
    ...
    */
    ...
    ?>
      <label for="instagram_activate">
        <input type="checkbox" id="instagram_activate" name="sk_social_media_hub[instagram][activate]" <?php checked( $activated, '1', true ); ?>> <?php _e('Aktivera Instagram', 'sk'); ?>
      </label>
      <?php
    }    
    ...
    public function instagram_user_id_feed() {
      ?>
        <input type="text" id="user_id_feed" size="60" name="sk_social_media_hub[instagram][user_id_feed]" value="<?php echo isset( $this->options['instagram']['user_id_feed'] ) ? esc_attr( $this->options['instagram']['user_id_feed'] ) : ''; ?>"> 
    ...
    public function instagram_client_id() {
      ?>
        <input type="text" id="instagram_client_id" size="60" name="sk_social_media_hub[instagram][client_id]" value="<?php echo isset( $this->options['instagram']['client_id'] ) ? esc_attr( $this->options['instagram']['client_id'] ) : ''; ?>"> 
    ...
    public function instagram_access_token_callback() {
      ...
      ?>
        <input type="text" id="access_token" size="60" name="sk_social_media_hub[instagram][access_token]" value="<?php echo $instagram_access_token; ?>"> 
    ...
    */
    ...
    ?>
      <label for="facebook_activate">
        <input type="checkbox" id="facebook_activate" name="sk_social_media_hub[facebook][activate]" <?php checked( $activated, '1', true ); ?>> <?php _e('Aktivera Facebook', 'sk'); ?>
      </label>
      <?php
    }    
    ...
    public function facebook_app_id() {
      ?>
        <input type="text" id="facebook_app_id" size="60" name="sk_social_media_hub[facebook][app_id]" value="<?php echo isset( $this->options['facebook']['app_id'] ) ? esc_attr( $this->options['facebook']['app_id'] ) : '' ?>"> 
    ...
    public function facebook_app_secret() {
      ?>
        <input type="text" id="facebook_app_secret" size="60" name="sk_social_media_hub[facebook][app_secret]" value="<?php echo isset( $this->options['facebook']['app_secret'] ) ? esc_attr( $this->options['facebook']['app_secret'] ) : '' ?>"> 
    ...
    public function facebook_access_token_callback() {
      ?>
        <input type="text" id="facebook_access_token" size="60" name="sk_social_media_hub[facebook][access_token]" value="<?php echo isset( $this->options['facebook']['access_token'] ) ? esc_attr( $this->options['facebook']['access_token'] ) : '' ?>"> 
        <a id="get-facebook-token" href="<?php echo $link; ?>" class="button button-primary"><?php _e( 'Hämta åtkomstkod', 'sksmh' ); ?></a>
    ...
    public function facebook_user_id_feed() {
      ?>
        <input type="text" id="user_id_feed" size="60" name="sk_social_media_hub[facebook][user_id_feed]" value="<?php echo isset( $this->options['facebook']['user_id_feed'] ) ? esc_attr( $this->options['facebook']['user_id_feed'] ) : ''; ?>"> 
    ...
    */
    ...
    ?>
      <label for="twitter_activate">
        <input type="checkbox" id="twitter_activate" name="sk_social_media_hub[twitter][activate]" <?php checked( $activated, '1', true ); ?>> <?php _e('Aktivera Twitter', 'sk'); ?>
      </label>
      <?php
    }    
    ...
    public function twitter_access_token_callback() {
      ...
      ?>
        <input type="text" id="twitter_access_token" size="60" name="sk_social_media_hub[twitter][access_token]" value="<?php echo $this->options['twitter']['access_token']?>"> 
        <a id="get-twitter-token" href="<?php echo $link; ?>" class="button button-primary"><?php _e( 'Hämta åtkomstkod', 'sksmh' ); ?></a>
        <p class="description"><?php _e( 'Applikations-ID och applikationshemlighet måste finnas angiven och sparad innan du kan hämta en åtkomstkod.', 'sksmh' ); ?></p>
        <?php
    }
    ...
    public function twitter_consumer_key_callback() {
      ?>
        <input type="text" id="consumer_key" size="60" name="sk_social_media_hub[twitter][consumer_key]" value="<?php echo isset( $this->options['twitter']['consumer_key'] ) ? esc_attr( $this->options['twitter']['consumer_key'] ) : '' ?>"> 
    ...
    public function twitter_consumer_secret_callback() {
      ?>
        <input type="text" id="consumer_secret" size="60" name="sk_social_media_hub[twitter][consumer_secret]" value="<?php echo isset( $this->options['twitter']['consumer_secret'] ) ? esc_attr( $this->options['twitter']['consumer_secret'] ) : '' ?>"> 
    ...
    public function twitter_user_id_feed() {
      ?>
        <input type="text" id="user_id_feed" size="60" name="sk_social_media_hub[twitter][user_id_feed]" value="<?php echo isset( $this->options['twitter']['user_id_feed'] ) ? esc_attr( $this->options['twitter']['user_id_feed'] ) : ''; ?>"> 
        <p class="description"><?php _e( 'Ange användarens id att hämta flöden ifrån, separera flera med kommatecken.', 'sksmh' ); ?></p>
        <div class="more">
  ...
  public function get_twitter_token(){
    ...
    $header = array( 'Authorization: Basic ' . $api_credentials, 'Content-Type: application/x-www-form-urlencoded;charset=UTF-8');
